# L293D Motor Driver PCB

## Overview
A custom motor driver PCB designed for controlling DC motors using the **L293D H-Bridge motor driver**. 
The board was designed as part of an embedded systems and electronics development project.

## Features
Dual DC motor control using L293D
H-Bridge motor driver configuration
74HC595 shift register for digital output expansion
External power and motor connection interfaces
Custom schematic and PCB layout

## Tools & Technologies
**KiCad**
PCB Design
Electronic Circuit Design
L293D H-Bridge Motor Driver
74HC595 Shift Register
